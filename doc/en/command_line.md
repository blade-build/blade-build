...
- **MSVC** has its *own* sample-based PGO — **SPGO** ([Sample Profile Guided Optimization](https://devblogs.microsoft.com/cppblog/introducing-sample-profile-guided-optimization-in-msvc/), VS 2022 / 2026 with MSVC 14.51+): collect with `xperf` (IP sampling on any CPU, LBR on Intel Haswell+/AMD Zen 4+/ARM64 ARMv9.2-A+), convert with `SPDConvert`, then `cl /GL /O2 /Zi … /link /debug /spgo` (collect build) → `… /link /LTCG /spdin:app.spd` (optimize build). It's a *different* mechanism from LLVM AutoFDO, so Blade's `--autofdo-*` (which emits the gcc/clang `-fprofile-sample-use` / `-fauto-profile` flags) is **skipped with a warning on MSVC and does not drive SPGO** — run SPGO manually for now, or use instrumentation PGO (`--profile-generate`/`--profile-use`).

**Platform availability (collection vs use).** LLVM AutoFDO *collection* (the gcc/clang path) needs `perf` + hardware **LBR**, i.e. a **bare-metal / PMU-passthrough x86_64 Linux** host — ARM Linux VMs typically expose no PMU/LBR, so `perf record -b` fails there. **Windows** has a separate sample-PGO path (MSVC SPGO via `xperf`, above) — not wired into Blade yet. **macOS has no sample-PGO collection path at all** (no `perf`, no SPGO; Instruments/`sample`/`dtrace` don't feed `llvm-profgen`). The `--autofdo-use` flag is portable, so a Linux-collected profile *can* be applied elsewhere, but cross-OS/arch reuse is imperfect and unsupported. On platforms where you can't collect, `--autofdo-generate` builds but is pointless — **use instrumentation PGO instead**: it is fully functional everywhere with no special hardware.
