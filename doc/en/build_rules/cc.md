...
- `export_map`: str, a single [linker "version" script](https://sourceware.org/binutils/docs/ld/VERSION.html) used to control which symbols the shared library exports.
  ...
  > **Cross-platform.** On Linux it is passed straight to GNU ld. On **MSVC** there is no `--version-script`, so Blade applies the map itself: it filters the auto-generated `.def` (see [Windows DLL support](#windows-dll-support)) by matching each symbol's **demangled name** (`UnDecorateSymbolName`) against the `global`/`local` patterns. Because that demangling is *name-only*, two limitations apply on MSVC: **overloads collapse** (a name is exported with all its overloads or none), and a quoted signature pattern (`"f(int)"`) is matched by its **name part only** (with a one-time warning). Full ELF symbol versioning (version nodes) is Linux-only. On **macOS (Apple ld64)** the situation is currently worse: ld64 understands neither `--version-script` nor a syntactically equivalent option (its native `-exported_symbols_list` takes a different file format), so **`export_map` is not yet honored**: Blade emits a warning and ignores the map, leaving the library at its default (fully exported). If you need to hide symbols on macOS, use `__attribute__((visibility("hidden")))` in source paired with `-fvisibility=hidden`.
