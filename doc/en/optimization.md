...
- **Keep LTO, export the symbols.** Add `-Wl,-export_dynamic` (macOS / ld64) or `-rdynamic` (ELF) to that binary's link — exporting all globals stops LTO from internalizing/dead-stripping the registrations, so they survive (verified to fix a real DI-registry binary under `--lto`). The cost is some of LTO's *internalization*-based wins (it can no longer prove globals are private to shrink/devirtualize them) and a larger dynamic symbol table; you keep cross-module **inlining**, which is most of LTO's runtime value.
