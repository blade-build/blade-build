...
**Self-registration / plugin patterns (a common gotcha).** LTO's whole-program dead-code elimination can drop **static-initializer registrations that are only reached by name at runtime** — factory / plugin / dependency-injection registries, and self-registering objects in general (the `[[gnu::constructor]]` + register-into-a-map idiom). From LTO's whole-program view the registrar is unreferenced, so it — and its `Register(...)` side effect — is stripped; the runtime lookup then fails (`… not found` / a registry `CHECK`) even though the program builds and passes **without** LTO. This is a property of the *code pattern*, not a blade or compiler defect, and it can fail a lot of tests at once if a project's DI mechanism uses it. Note that marking the registrar `__attribute__((used))` is **not always sufficient** (the registry singleton can also be split across translation units under LTO). Two ways out:

- **Keep LTO, export the symbols.** Exporting all globals stops LTO from internalizing/dead-stripping the registrations, so they survive (verified to fix a real DI-registry binary under `--lto`). Blade exposes this as **`cc_config(lto_export_dynamic = True)`**, which adds `-Wl,-export_dynamic` (macOS) / `-rdynamic` (ELF) **only when LTO is active** — so non-LTO builds pay nothing. (You can also add the flag by hand via `linkflags` / `extra_linkflags`.) The cost under LTO is some of its *internalization*-based wins (it can no longer prove globals are private to shrink/devirtualize them) and a larger dynamic symbol table; you keep cross-module **inlining**, which is most of LTO's runtime value.
- **Or skip LTO for that binary** — `lto = False` on the target (or `--lto=no`) until the registry is made LTO-safe (a single strong-linkage instance).
