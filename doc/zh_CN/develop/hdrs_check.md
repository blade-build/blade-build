...
**边界——信任止于生成目标。** 间接检查只验证使用方**声明了对所包含生成头的依赖**，并**不会**下钻到该生成头自身的 `#include`。后者被信任为由*生成它的*目标在自身编译时覆盖——例如 `proto_library` 会编译其 `.pb.cc`，因此 `.pb.h` 的包含在那个上下文里已被 `-H` 检查。于是，一个**只产出裸头文件、却不编译任何翻译单元**的 `gen_rule` 就是已知的盲点：若这种头自身又 `#include` 了另一个项目头文件，该传递依赖不会被强制检查。实践中风险很低，因为生成头很少包含项目头文件——它们引入的是自己的运行时（`google/protobuf/...`、标准库），而非你的库。如果确实遇到这种情况，请在使用方（或 `gen_rule` 的 `deps`）显式声明该依赖。
