...
`runall.sh` 运行的用例集来自 `blade_main_test.py` 里显式的 `TEST_CASES`
列表，因此新增的集成测试类必须加进去才会在 CI 里运行。
`tests/unit/integration_suite_coverage_test.py` 会守护这一点：只要有
`src/test/*_test.py` 里定义了 `test*` 方法的类没被登记进 `TEST_CASES`，
它就会失败——集成测试不会再被悄悄漏掉。
