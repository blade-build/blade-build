...
class IsAutogenBuildSkipTest(unittest.TestCase):
    """Blade's own build dirs (the ``.bladeskip`` blade auto-drops) are skipped
    silently; a user-created ``.bladeskip`` is still announced. The two are told
    apart by the ``Auto-generated by blade`` marker blade writes into its own
    sentinel -- content, not directory name."""

    def _skip_file(self, tmp, content):
        path = os.path.join(tmp, '.bladeskip')
        with open(path, 'w', encoding='utf-8') as f:
            f.write(content)
        return path

    def test_autogen_marker_detected(self):
        with tempfile.TemporaryDirectory() as tmp:
            content = (f'# {load_build_files._AUTOGEN_SKIP_MARKER} to mark this '
                       'directory as a build output. Safe to leave in place.\n')
            self.assertTrue(
                load_build_files._is_autogen_build_skip(self._skip_file(tmp, content)))

    def test_user_skip_not_autogen(self):
        with tempfile.TemporaryDirectory() as tmp:
            self.assertFalse(
                load_build_files._is_autogen_build_skip(self._skip_file(tmp, 'my manual skip\n')))

    def test_empty_skip_not_autogen(self):
        with tempfile.TemporaryDirectory() as tmp:
            self.assertFalse(
                load_build_files._is_autogen_build_skip(self._skip_file(tmp, '')))

    def test_missing_file_not_autogen(self):
        with tempfile.TemporaryDirectory() as tmp:
            self.assertFalse(
                load_build_files._is_autogen_build_skip(os.path.join(tmp, '.bladeskip')))
