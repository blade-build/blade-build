...
# Token blade writes into the `.bladeskip` it auto-drops in its own build output
# directory (see Workspace.setup_build_dir). It tells blade's own build dirs
# (skipped silently -- there can be one per profile, and announcing each is just
# noise) from a user-designated skip directory (still announced, so the user can
# see their skip is taking effect).
_AUTOGEN_SKIP_MARKER = 'Auto-generated by blade'
...
def _has_load_excluded_file(root, files):
    ...
    if _BLADE_SKIP_FILE in files:
        if not _is_autogen_build_skip(os.path.join(root, _BLADE_SKIP_FILE)):
            console.info(f'Skip "{root}" due to the "{_BLADE_SKIP_FILE}" file')
        return True
    return False


def _is_autogen_build_skip(skip_path):
    """Whether a `.bladeskip` was auto-dropped by blade into its own build output
    directory, as opposed to created by the user. Build dirs are skipped
    silently; user-designated skips are announced."""
    try:
        with open(skip_path, 'r', encoding='utf-8') as f:
            return _AUTOGEN_SKIP_MARKER in f.read(256)
    except OSError:
        return False
