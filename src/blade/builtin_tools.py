...
def archive_package_sources(package, sources, destinations):
    ...
    for i, s in enumerate(sources):
        ...
        manifest.append('{} {}'.format(util.md5sum_file(s), destinations[i]))
    return manifest
...
def _generate_resource_index(targets, sources, name, path):
    ...
    with open(header, 'w') as h, open(source, 'w') as c:
        ...
        for s in sources:
            ...
            c.write('    {{ "{}", RESOURCE_{}, {} }},\n'.format(entry_name, entry_var, entry_size))
...
def generate_java_jar(compression_level, args):
    ...
    def archive_resources(resources_dir, resources, new=True):
        ...
        cmd = ['{} {} {}'.format(jar, options, target)]
        for resource in resources:
            cmd.append("-C '{}' '{}'".format(resources_dir,
...
def generate_shell_testdata(args):
    ...
    with open(path, 'w') as f:
        for i in range(middle):
            f.write('{} {}\n'.format(os.path.abspath(sources[i]), destinations[i]))
...
def main():
    ...
    try:
        ...
        console.error('Blade build tool {} error: {} {}'.format(name, str(e), traceback.format_exc()))
        exit_code = 1
    finally:
        if exit_code:
            _cleanup_outputs()
