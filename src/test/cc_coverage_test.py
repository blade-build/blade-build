...
(`build_release_coverage`), instruments the cc targets, and -- when gcovr
is available -- produces an HTML report. The plain `build_release` name is
left untouched.
"""
...
class TestCcCoverage(blade_test.TargetTest):
    ...
    COVERAGE_BUILD_DIR = 'build_release_coverage'
    ...
    def doTearDown(self):
        # The base harness only cleans build_release; remove the coverage
        # sibling dir too.
        shutil.rmtree(self.COVERAGE_BUILD_DIR, ignore_errors=True)
